...
func (cl *Clients) GetByListener(id string) []*Client {
	...
	// cl.RLock() above, and Len() acquires it again. That recursive read-lock
	// deadlocks permanently whenever a concurrent Lock() (e.g. Clients.Add or
	// Clients.Delete, called on every connect/disconnect) is pending at the
	// moment the second RLock is attempted -- Go's sync.RWMutex blocks new
	// readers once a writer is waiting, specifically to prevent writer
	// starvation, which is exactly what turns this into a deadlock instead of
	// mere contention. See https://github.com/mochi-mqtt/server/issues/488.
